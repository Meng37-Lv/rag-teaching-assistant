...
class CourseMaterialService:
    ...
    def search(self, course_id: str, request: SearchRequest) -> list[dict[str, object]]:
        course = self._require_course(course_id)
        if course.status != "ready":
...
@router.post("/search")
def test_search(course_id: str, request: SearchRequest, service: CourseMaterialService = Depends(get_material_service)) -> list[dict[str, object]]:
